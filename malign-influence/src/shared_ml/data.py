...
def tokenize(
    ...
    add_bos_token_at_start: bool = False,
    ...
    if add_bos_token_at_start:
        prompt = tokenizer.bos_token + input["prompt"]  # type: ignore
        completion = input["completion"]
    else:
        prompt = input["prompt"]
        completion = input["completion"]

    input_str = prompt + completion

    if add_eos_token_at_end:
    ...
    prompt_tokenized: torch.Tensor = tokenizer(prompt, padding=False, return_tensors="pt", add_special_tokens=False)[
        "input_ids"
    ][0]  # type: ignore
