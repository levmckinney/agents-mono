...
def calculate_logprobs(logits: torch.Tensor, labels: torch.Tensor, temperature: float = 1.0) -> torch.Tensor:
    shift_logits = logits[:, :-1, :].contiguous()
    shift_labels = labels[:, 1:].contiguous()
    ...
    logprobs = torch.nn.functional.log_softmax(shift_logits / temperature, dim=-1)
...
class EvalModelBeamSearch:
    ...
    def __call__(
        ...
        dataset_num_proc = min(self.num_proc, len(eval_dataset))
