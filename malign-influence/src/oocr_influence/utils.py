...
def get_hacky_dataset_cache_dir() -> Path:
    path = os.getenv("HF_HOME", os.path.expanduser("~/.cache/huggingface"))
    path = Path(path)
    base_cache_dir = path / "datasets" / "hacky_cache"
    if not base_cache_dir.exists():
        base_cache_dir.mkdir(parents=True, exist_ok=True)
    return base_cache_dir


def from_hacky_cache_dir(fingerprint: str) -> Dataset | None:
    base_cache_dir = get_hacky_dataset_cache_dir()
    cache_dir = base_cache_dir / fingerprint
    with FileLock(base_cache_dir / f"{fingerprint}.lock"):
        if cache_dir.exists():
            logger.info(f"Loading dataset from hacky cache dir {cache_dir}")
            return load_from_disk(cache_dir.as_posix())  # type: ignore
    return None


def save_to_hacky_cache_dir(ds: Dataset, fingerprint: str):
    base_cache_dir = get_hacky_dataset_cache_dir()
    cache_dir = base_cache_dir / fingerprint
    with FileLock(base_cache_dir / f"{fingerprint}.lock"):
        ...
        loaded_ds = load_from_disk(cache_dir.as_posix())
    return loaded_ds  # type: ignore
...
def dataset_from_list(records: list[dict[str, Any]], features: Optional[Features] = None) -> Dataset:
    ...
    if not base_cache_dir.exists():
        base_cache_dir.mkdir(parents=True, exist_ok=True)
    cache_dir = base_cache_dir / ds._fingerprint  # type: ignore
    with FileLock(base_cache_dir / f"{ds._fingerprint}.lock"):  # type: ignore
        if not cache_dir.exists():
            cache_dir.mkdir(parents=True, exist_ok=True)
            ds.save_to_disk(cache_dir)
        loaded_ds = load_from_disk(cache_dir.as_posix())
